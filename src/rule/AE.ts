...
export class AE {
  public static get requirements(): BoolExpr[] {
    const lectures: LecturesForValidation = Object.fromEntries(
      Object.entries(BaseExpr.lectures).map(([key, value]) => [
        key,
        new LecsExpr(value),
      ])
    ...
    const graduationRequirements: BoolExpr[] = [
      ...
      If(
        AEis.복전,
        lectures.기선.credit.atLeast(6),
        lectures.기선.credit.atLeast(9)
      ),
      ...
      If(
        ...
        And(
          ...
          lectures.심전필
            .approve(
              lectures.all
                .filter({ codeGroup: "500단위" })
                .filter({ group: "학사대학원상호인정" })
            )
            .credit.atLeast(18)
        )
      ),
      ...
      If(
        AEis.부전,
        And(lectures.전필.credit.atLeast(9), lectures.all.credit.atLeast(18))
      ...
      If(
        AEis.복전,
        And(lectures.all.credit.atLeast(42), lectures.전필.credit.atLeast(21))
        // TODO ※타 학사조직 전공과목 최대 6학점까지 중복인정 가능 // 이건 give가 가능하다는거고?
      ),

      // AE가 포함된 track : 연구 // 연구는 원래 주전공만 하는 거임?
      If(
        new BoolExpr(BaseExpr.user.track.복전.length !== 0).setMsg(
          "복전이 하나라도 존재"
        ),
