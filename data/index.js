...
function getKeyIfCommon(array) {
    ...
    array.forEach((element) => {
        assert.deepStrictEqual(
            increasingOrder(Object.keys(element)),
            increasingOrder(Array.from(keys.values())));
    });
    return keys;
}
...
function classify(array) {
    ...
    assert.ok(array.every(element => element !== undefined), "All element in array should have value");
    assert.ok(array.every(element => element === null || typeof possibleType.includes(element)), "All element in array should have proper type");
...
function creditChangeOverTimeForSameCode() {
    ...
    console.log(yearsSortedByCredit_SortedByCode
        .filter(([code, yearsSortedByCredit]) => yearsSortedByCredit.length != 1)
        .map(([code, yearsSortedByCredit]) => `${code}${yearsSortedByCredit.map(([credit, years]) => `\n\t${credit} - ${years.join(", ")}`).join("")}`).join("\n")
    );
    //
}
